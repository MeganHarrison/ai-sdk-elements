...
// .wrangler/tmp/bundle-bbdufi/checked-fetch.js
...
// .wrangler/tmp/bundle-bbdufi/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-bbdufi/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
var src_default = {
  async fetch(request, env) {
    ...
    try {
      const { data: recentMeetings, error: meetingsError } = await supabase.from("meetings").select(`
          id,
          title,
          date,
          project_id,
      ...
      for (const meeting of recentMeetings || []) {
        const { data: existingInsights } = await supabase.from("ai_insights").select("id").eq("meeting_id", meeting.id).limit(1);
        if (existingInsights && existingInsights.length > 0) {
          continue;
        }
        const { data: chunks, error: chunksError } = await supabase.from("meeting_chunks").select("content, chunk_type, speaker_info, start_timestamp, end_timestamp").eq("meeting_id", meeting.id).order("chunk_index", { ascending: true });
        if (chunksError) {
        ...
        const fullTranscript = chunks.map((chunk) => {
          const speaker = chunk.speaker_info ? typeof chunk.speaker_info === "object" && "name" in chunk.speaker_info ? chunk.speaker_info.name : "Speaker" : "Speaker";
        ...
        for (const insight of meetingInsights) {
          const { error: insertError } = await supabase.from("ai_insights").insert({
            ...
            source_meetings: meeting.id,
            // Single meeting as source
            resolved: 0
          });
          if (insertError) {
...
async function determineProjectContext(meeting, fullTranscript, supabase) {
  ...
  const projectPatterns = [
    ...
    /working\s+on\s+["']?([^"'\n]+?)["']?\s/gi,
    /regarding\s+["']?([^"'\n]+?)["']?\s+project/gi,
    /(\d{3,6})\s+(?:project|job)/gi
    // Job numbers
  ];
  const transcriptSample = fullTranscript.substring(0, 5e3);
...
__name(determineProjectContext, "determineProjectContext");
async function generateMeetingInsights(fullTranscript, meeting, projectContext, openaiKey) {
  ...
  const transcriptForAnalysis = fullTranscript.length > maxLength ? fullTranscript.substring(0, maxLength) + "\n...[transcript truncated]" : fullTranscript;
  try {
...
Extract 3-8 key insights from this meeting. Focus on the most important and actionable items.`
    ...
    return insights.filter(
      (insight) => insight.title && insight.description && insight.insight_type && insight.severity
    );
  } catch (error) {
...
__name(generateMeetingInsights, "generateMeetingInsights");
async function generateCrossReferenceInsights(supabase, openaiKey) {
  const sevenDaysAgo = /* @__PURE__ */ new Date();
  sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);
  const { data: recentInsights, error } = await supabase.from("ai_insights").select(`
      *,
      meetings (
        title,
        date
  ...
  const projectInsights = /* @__PURE__ */ new Map();
  ...
  for (const [projectId, insights] of projectInsights) {
    ...
    const { data: existingPattern } = await supabase.from("ai_insights").select("id").eq("project_id", projectId).like("title", "[PATTERN]%").gte("created_at", sevenDaysAgo.toISOString()).limit(1);
...
${insights.map(
                (i) => `[${i.severity}] ${i.title} (${i.meetings?.date || "Unknown date"})
   ${i.description}`
              ).join("\n\n")}
...
Identify significant patterns or trends across these insights.`
      ...
      if (response.ok) {
        ...
        if (pattern.has_pattern) {
          const meetingIds = [...new Set(insights.map((i) => i.meeting_id).filter(Boolean))];
      ...
      console.error(`Error generating pattern for project ${projectId}:`, error2);
    }
  }
}
__name(generateCrossReferenceInsights, "generateCrossReferenceInsights");
